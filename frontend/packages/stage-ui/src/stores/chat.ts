  ...
  async function streamWithStageAdapters(
    ...
    if (typeof window !== 'undefined' && (window as any).__betterAgentWSBridge) {
      ...
      const promptText = typeof lastUserMsg?.content === 'string'
        ? lastUserMsg.content
        : (Array.isArray(lastUserMsg?.content) ? (lastUserMsg?.content[0] as any)?.text || '' : '')

      if (promptText) {
        console.log('[BetterAgent] Redirecting chat prompt to BetterAgent Python CognitiveEngine:', promptText)

        // Trigger force vision snapshot capture if vision is active
        if (typeof window !== 'undefined' && typeof (window as any).__triggerVisionForceCapture === 'function') {
          try {
            (window as any).__triggerVisionForceCapture()
          } catch (e) {
            console.warn('[BetterAgent] Error triggering force vision capture:', e)
          }
        }

        return new Promise<void>((resolve) => {
          let finishTimer: any = null
          let textUnsub: (() => void) | null = null
          let emotionUnsub: (() => void) | null = null

          const cleanup = () => {
            if (finishTimer) clearTimeout(finishTimer)
            if (textUnsub) textUnsub()
            if (emotionUnsub) emotionUnsub()
          }
          ...
          emotionUnsub = bridge.onEmotion((emotion: string) => {
            console.log('[BetterAgent] Emotion received for turn:', emotion)
          })

          // Send prompt via WebSocket
          bridge.sendUserText(promptText)

          // Fallback timeout after 30s if backend fails to respond
          setTimeout(() => {
            cleanup()
            resolve()
          }, 30000)
