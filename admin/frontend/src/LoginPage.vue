...
async function submit() {
  ...
  loading.value = true
  try {
    // 用输入的密钥请求后端 API 验证；后端返回 200 即通过，401 即密钥错误
    const resp = await fetch('/api/admin/personas', {
      headers: { 'Content-Type': 'application/json', 'X-Admin-Token': secret },
    })
    if (resp.ok) {
      localStorage.setItem('betteragent/admin-token', secret)
      emit('authed')
    }
    else {
      errorMsg.value = '访问密钥错误'
    }
  }
  catch {
    errorMsg.value = '无法连接后端服务，请稍后重试'
  }
  finally {
    loading.value = false
  }
    ...
    <div class="login-shell">
      ...
      <section class="card">
        ...
        <form class="form" @submit.prevent="submit">
          <label class="field">
            <span>访问密钥</span>
            <input v-model="form.secret" type="password" placeholder="请输入 ADMIN_SECRET_KEY" autocomplete="current-password" />
          </label>
...
.card-head { display: flex; flex-direction: column; align-items: center; text-align: center; margin-bottom: 26px; }
